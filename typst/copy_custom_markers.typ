...
#let header = (circle-diameter) => {
  ...
  context if generating-content.get() {
    ...
    grid(
      ...
      column-gutter: 1fr,
      {
        gen-svg-box("hztl", SVG, circle-diameter)
      },
      {
        gen-svg-box("hztr", rotate(90deg,SVG), circle-diameter)
      }
    )
  } else []
}
...
// ----------------------------------------------------------------
#let footer = (circle-diameter, exam-id, barcode-type, barcode-height) => {
  set align(bottom)
  context if generating-content.get() {
    grid(
      columns: 3,
      ...
      {
        set align(bottom)
        gen-svg-box("hzbl", rotate(-90deg,SVG), circle-diameter)
      },
      {
        grid(
          columns: 1,
...
#let gen-copies(
  ...
  set page(
    ...
    header-ascent: 0mm,
    header: { header(circle-diameter) },
    footer-descent: 0mm,
    footer: context [
      #set align(bottom)
      #footer(circle-diameter, exam-id, barcode-type, barcode-height)
    ],
  )
...
#let c-d = float(sys.inputs.at("circle-diameter", default: "4")) * 1mm
