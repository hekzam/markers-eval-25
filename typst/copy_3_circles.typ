...
#let footer = (circle-diameter, exam-id, barcode-type, barcode-height) => {
  ...
  context if generating-content.get() {
    grid(
      ...
      {
        let (copy-i, page-i) = get-indexes() // EXAM-CONTENT-HASH, exam-id
        barcode("hzbr", ("hzbr", str(copy-i), str(page-i)), barcode-height, barcode-height, type: barcode-type)
      },
    )
    v(MARGIN_Y)
  } else []
...
#let gen-copies(
  ...
  set page(
    ...
    footer-descent: 0mm,
    footer: { footer(circle-diameter, exam-id, barcode-type, barcode-height) },
  )
