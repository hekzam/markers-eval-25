...
analyser_donnees_copies <- function(chemin_fichier) {
  ...
  temps_moyen <- mean(donnees$Temps_sec, na.rm = TRUE)
  ecart_type <- sd(donnees$Temps_sec, na.rm = TRUE)
...
donnees_triees <- donnees %>% arrange(Temps_sec)
print(donnees_triees)

# Statistiques complémentaires
cat("\nStatistiques complémentaires :\n")
cat("Temps moyen :", mean(donnees$Temps_sec), "sec\n")
cat("Temps médian :", median(donnees$Temps_sec), "sec\n")
cat("Écart-type :", sd(donnees$Temps_sec), "sec\n")
cat("Copie la plus rapide :", donnees$File[which.min(donnees$Temps_sec)], "\n")
cat("Copie la plus lente :", donnees$File[which.max(donnees$Temps_sec)], "\n")
